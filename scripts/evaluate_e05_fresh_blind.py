...
CANDIDATE_PATH = Path("src/evaluation/calibrated_e05.py")
...
def main() -> int:
    p = argparse.ArgumentParser(description="Score frozen E0.5 exactly once on locked fresh blind labels")
    ...
    if payload.get("schema_version") != "E05_FRESH_BLIND_LABELS_V1.0.0":
        raise SystemExit("unexpected fresh blind label schema")
    if payload.get("status") != "LABELS_LOCKED_BEFORE_E05_SCORING":
    ...
    for cid in sorted(labels):
        ...
        if pred != expected:
            ...
            if expected in SURFACED and pred == "SKIP":
    ...
    out = {
        "schema_version": "E05_FRESH_BLIND_SCORE_V1.0.0",
        ...
        "decision": "PROMOTION_ELIGIBLE" if not failures else "REJECT_E0.5_FOR_PROMOTION",
    }
